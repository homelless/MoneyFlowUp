...
import SwiftData
    ...
    // Экран списка транзакций в кошельке с возможностью:
    ...
    struct AccountTransactionsView: View {
        ...
        // Идентификатор аккаунта, для которого показываем транзакции
        ...
        // Отфильтрованные транзакции по текущему интервалу, группе и текущему аккаунту
        var filteredTransactions: [Transaction] {
            ...
            // Фильтр по текущему аккаунту
            filtered = filtered.filter { tx in
                if tx.isTransfer {
                    // Для переводов учитываем обе стороны
                    return tx.account?.id == accountID || tx.toAccount?.id == accountID
                } else {
        ...
        var body: some View {
            ...
            .navigationTitle("\(accountVM.accounts.first(where: { $0.id == accountID })?.name ?? "Кошелек")")
